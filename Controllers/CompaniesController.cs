    ...
    public class CompaniesController : ControllerBase
    {
        ...
        public async Task<ActionResult<CompanyDto>> CreateCompany([FromBody] CreateCompanyDto dto)
        {
            ...
            try
            {
                ...
                // Ownership: record who created the company. Written straight
                // on the entity so CreateCompanyDto and CompanyDto stay exactly
                // as they were -- no contract change.
                if (CurrentUserId > 0)
                {
                ...
                {
                    // Grant the creator AND everyone above the creator in the
                    // CreatedBy chain (seed admin excluded -- it needs no row).
                    // Whoever manages the creator can then see and administer
                    ...
                    {
                        await _context.SaveChangesAsync();
                        // Drop the cached accessible-sets so the next request
                        // sees the new grants immediately instead of waiting
                        // out the 60s sliding TTL.
