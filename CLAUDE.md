...
### 5b-2b. The catalog is global in STORAGE, scoped in VISIBILITY (2026-09-09)

`ItemType` still has no `CompanyId` and must not grow one — the HS tariff master
lives in that table and is installation-wide reference data. But an installation
serves unrelated tenants, and every picker was listing every tenant's product
names, because passing `companyId` only ever changed the SORT.

- **Membership is DERIVED, not owned.** `ItemTypeRepository.CompanyItemTypeIds`
  unions the company's `CompanyItemTypeSettings` registrations with every table
  that references an item: openings, stock movements, invoice lines and their
  filed-book adjustments, purchase lines, debit notes, goods receipts, challan
  lines, order lines, quote lines. Adding a new document type that carries an
  `ItemTypeId` means adding a leg here, or its items vanish from their owner's
  picker.
- **It is an `IQueryable`, composed into the caller's query as one
  `IN (SELECT …)`.** Materialising thousands of ids and passing them back as
  parameters would not survive the paged screen, which filters, counts and
  orders in SQL.
- **NULL and EMPTY mean different things.** Null is an internal caller (a
  seeder, an importer, the parameterless overload) asking for the installation.
  An empty list is a USER who can reach no company: they get the tariff and
  nothing else, never everything. Getting this backwards turns the guard into a
  no-op for exactly the caller it exists for.
- **The scope is the caller's ACCESSIBLE SET, not one company.** `GET
  /api/itemtypes` without a `companyId` already aggregated on-hand across
  `GetAccessibleCompanyIdsAsync`; the visibility filter reuses that set, so
  omitting the parameter can no longer widen the answer past the caller's
  tenancy.
- **An un-adopted HS placeholder (`IsAutoGenerated && !IsFavorite`) stays
  visible to everyone.** It is FBR's published tariff, owned by no tenant, and
  hiding it would stop a company classifying its first item. Adoption
  (`IsFavorite`) makes a placeholder that company's own row and scopes it — the
  same adoption signal §5b-2 already turns on.
- **Creating an item REGISTERS it.** `EnsureRegisteredAsync` writes a
  `CompanyItemTypeSetting` with `Mode = Default` whenever a create or update
  carries a companyId, independent of `WriteCompanyOverlay`. Membership is
  otherwise derived from documents and a new item has none, so without this the
  company that typed an item could not see it in the picker it was typed into.
  `Mode = Default` is ignored by `GetStockTrackedItemTypeIdsAsync`, so this
  records ownership and changes no tracking behaviour.
- Suite: `scripts/test_tenant_isolation.py` suite 18 — and it is deliberately
