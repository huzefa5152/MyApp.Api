...
namespace MyApp.Api.Middleware
{
    ...
    /// before any public portal action runs, and refuses the request when it
    /// does not resolve.
    ///
    /// A filter rather than a line at the top of each action, for the same
    /// reason <see cref="AuthorizeCompanyAttribute"/> exists: the check has to
    /// be impossible to forget on the one endpoint somebody adds later. Actions
    /// read the result out of <c>HttpContext.Items</c>.
    ///
    /// Every failure — unknown token, disabled portal, revoked portal, malformed
    /// token — returns the SAME 404 with the SAME body.
    /// <c>GlobalExceptionMiddleware</c> echoes 4xx messages verbatim, so distinct
    /// wording would tell a stranger which of their guesses had once been a real
    /// portal.
    /// </summary>
    ...
    internal class ResolvePortalFilter : IAsyncAuthorizationFilter
    {
        ...
        public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
        {
            ...
            // The audit trail derives its tenant from a route {companyId}, a
            // ?companyId=, or this key. A token-only route matches neither of
            // the first two, so without this line the entire public surface
            // would be untenanted in the audit log.
