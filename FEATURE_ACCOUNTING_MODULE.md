...
| 2026-09-17 | 5 | **Customer Portal DONE.** `CustomerPortal` (migration `AddCustomerPortal`), `CustomerPortalService`, `CustomerPortalsController` + `PublicCustomerPortalController`, `ResolvePortalAttribute`, `PublicTokenGenerator`, `PortalTokenLogMasker` (wired as a Serilog enricher), a `portal` rate-limit policy partitioned on the token, `customerportals.manage.*`, `CustomerPortalsPage.jsx`, `public/PublicPortalPage.jsx`. New suite `test_customer_portal.py` 73/73 — the IDOR suite, and a merge blocker; whole Trader table green. **Two bugs it caught:** company delete 500'd because the portal cleanup ran after the clients it references were deleted (moved to the front of the cascade); and the portal link 404'd into the landing page, because this build mounts the SPA under `/admin/` — fixed with a `portal/{*path:nonfile}` fallback plus rendering the page from `main.jsx` OUTSIDE the router and the auth providers, which also avoids a 401 storm bouncing an anonymous visitor to the operator login. **Deviation:** no bulk "download all" endpoint — it needs `IInvoiceBulkService`, which this line does not have, and a smaller anonymous surface is the right trade. |

| 2026-09-17 | 6 | **GL always-on + back-post DONE.** No toggle exists anywhere: `GlPostingEnabled` is set at company creation, is absent from every DTO so no request can write it, and is read-only in the status endpoint — Phase 2 already delivered that half. New: `Data/GlBackfill.cs`, the one-shot that brings companies predating the module up to date (chart if none, default accounts, flag on, then rebuild), guarded by an `AuditLog` marker `GL_BACKFILL_V1` like every other backfill here; per-company try/catch, and the marker is deliberately NOT written if any company failed so the next boot retries. Wired into `Program.cs` before the Demo seeder. It ran on this machine's Trader database at first boot: 9 companies back-posted, every company's ledger balanced. New suite `test_gl_backfill.py` 46/46; whole Trader table green (stock reflow 161/161, chart 103, GL 93, taxes 67, posting 82, reports 55, portal 73, admin scope 115, basic flows 37, exact line total 69, FBR cancellation 26, audit 67). **The suite caught a real bug:** `PostingService.RebuildAsync` removed the system-posted entries with `ExecuteDeleteAsync` — raw SQL, which the ledger's lock check never sees — and then re-posted through the writer, which does see it. A rebuild on a company with a lock date therefore DELETED the closed period and then refused to write it back: the one path in this module that could lose a filed figure. A rebuild now leaves closed-period entries alone and skips the documents behind them, and the whole rebuild runs in one transaction so a document that fails half-way cannot leave the books emptied. |
