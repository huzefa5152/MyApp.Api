...
| Editable per-line Account column | `PurchaseBillForm.jsx`, `InvoiceForm.jsx` (bill from challan), `StandaloneInvoiceForm.jsx` (bill without challan) — all gated on `glOn`, auto-fill from the item overlay, and name the resolved company default in the placeholder |
| Account-name display helper | `myapp-frontend/src/utils/accountDisplay.js` — `findAccountLabel` + `defaultAccountPlaceholder(accounts, defaultAccountId)` name the company-default account for the per-line picker placeholder |
| Company default accounts (read projection) | `DTOs/CompanyDto.cs` (`DefaultSalesAccountId`/`DefaultPurchaseAccountId`) + `Services/Implementations/CompanyService.cs` (`ToDto`) |
| Reusable GL account picker | `myapp-frontend/src/Components/AccountSelect.jsx` — generic Chart-of-Accounts dropdown (grouped: Income/Expense first via `side`, everything else under "Other accounts"). Caller passes the flat `accounts` list (fetched once via `getAccountsFlat(companyId)`) so it's safe per table row. Reused by `ItemTypeForm` (GL mapping), `NonInventoryItemForm`, and `PurchaseBillForm`'s per-line Account column — use it for any future GL-account field. |
...
- Editable per-line **Account** column on `EditBillForm` (bill/invoice edit). The sales-side P&L split still works there — it just derives the account from the item-type overlay instead of offering a manual per-line override.
- Moving item-type curation (`IsFavorite`/`UsageCount`) onto the overlay (still global on `ItemType`).

**Done (was deferred):**
- Editable per-line **Account (GL)** column on the sales bill-creation forms — `InvoiceForm` (bill from challan) and `StandaloneInvoiceForm` (bill without challan) — matching `PurchaseBillForm`. Auto-fills from the item type's overlay sale account, overridable, gated on the company having a CoA (`glOn = accounts.length > 0`); `accountId` rides the create payloads (backend already accepted it). A non-inventory line disables the picker and names the item's own mapped account; an unmapped line names the resolved company default (see below).
- **Company default account NAMED, not "— default —":** `CompanyDto` now exposes `DefaultSalesAccountId` / `DefaultPurchaseAccountId` (read-only projection, mapped in `CompanyService.ToDto`, pinned by `EnsureDefaultInventoryAccountsAsync`). The `AccountSelect` placeholder on all three bill forms resolves that id against the flat accounts list via `utils/accountDisplay.js` (`defaultAccountPlaceholder`) so an unmapped line reads `→ <account>` instead of a bare default label. Falls back to `— company default —` when the id isn't set.
- Division-filtered document pickers (design §7) — implemented via `SearchableItemTypeSelect`'s `divisionId` prop across all document forms; the picker also now separates Non-Inventory charges from Inventory items into two independently-scrolling panes.
