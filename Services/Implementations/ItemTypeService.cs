...
namespace MyApp.Api.Services.Implementations
{
    public class ItemTypeService : IItemTypeService
    {
        ...
        public async Task<List<ItemTypeDto>> GetAllAsync(
            ...
            int? take = null)
        {
            // ItemType is a global catalog, so without this every picker
            // listed every other tenant's product names.
            // What this caller may SEE, which is not the same question as
            // what on-hand to show them. One company when the caller named one;
            // otherwise the set they can reach, which the Item Catalog already
            // passes for its aggregate figures. Null only when an internal
            // caller supplied neither -- see ScopeToCompany.
            var scopeIds = companyId.HasValue
                ? new List<int> { companyId.Value }
                : aggregateAcrossCompanyIds?.ToList();
            var items = await _repo.GetAllAsync(includeAutoGenerated, search, take, scopeIds);
            ...
            var onHandByItem = await LoadOnHandAsync(
                items, companyId, aggregateAcrossCompanyIds, divisionRestrictionsByCompany);
        ...
        /// <summary>
        /// On-hand per item for a set of catalog rows. Aggregation mode wins
        /// when supplied — the Item Catalog screen passes the caller's
        /// accessible company set so on-hand is a global number rather than a
        /// per-company one. An empty set falls through to the per-company path,
        /// and a company without tracking gets no figures at all.
        /// </summary>
        private async Task<Dictionary<int, decimal>?> LoadOnHandAsync(
            List<ItemType> items,
            int? companyId,
            IEnumerable<int>? aggregateAcrossCompanyIds,
            Dictionary<int, HashSet<int>>? divisionRestrictionsByCompany)
        {
            ...
            {
                return await _stock.GetOnHandBulkAcrossCompaniesAsync(
            ...
            {
                // Same numbers the Stock Dashboard shows. Items with no
                // purchase/sale history simply have no row in the dictionary.
                HashSet<int>? divScope = null;
                divisionRestrictionsByCompany?.TryGetValue(companyId.Value, out divScope);
                return await _stock.GetOnHandBulkAsync(companyId.Value, ids, allowedDivisionIds: divScope);
            }

            return null;
        }

        public async Task<PagedResult<ItemTypeDto>> GetPagedAsync(
            string? search,
            int page,
            int pageSize,
            int? companyId = null,
            ...
            Dictionary<int, HashSet<int>>? divisionRestrictionsByCompany = null)
        {
            ...
            var scopeIds = companyId.HasValue
                ? new List<int> { companyId.Value }
                : aggregateAcrossCompanyIds?.ToList();
            var (rows, total) = await _repo.GetPagedAsync(search, page, pageSize, scopeIds);

            var onHandByItem = await LoadOnHandAsync(
                rows, companyId, aggregateAcrossCompanyIds, divisionRestrictionsByCompany);

            // The page keeps the ORDER the database gave it. Re-sorting here on
            // on-hand would shuffle rows within the page while the pages
            // themselves stayed in name order, which reads as broken sorting.
            var items = rows.Select(it =>
            {
                var dto = ToDto(it);
                if (onHandByItem != null)
                    dto.AvailableQty = onHandByItem.TryGetValue(it.Id, out var q) ? q : 0m;
            ...
            if (companyId.HasValue)
                await ApplyOverlayAsync(items, companyId.Value);
        ...
        public async Task<ItemTypeDto> CreateAsync(ItemTypeDto dto, int? companyId = null)
        {
            ...
            var created = await _repo.CreateAsync(new ItemType
            {
                Name = dto.Name,
                HSCode = normalizedHs,
                // Link to the HS master row when the code is one it knows, so
                // the catalog carries a real FK rather than only a loose string.
                // Null when the master has not been imported yet — HSCode above
                // stays the value FBR submission reads either way.
                HsCodeId = await ResolveHsCodeIdAsync(normalizedHs),
                UOM = dto.UOM,
            ...
            if (companyId.HasValue && dto.WriteCompanyOverlay)
                ...
                // Register it even when no overlay fields were sent. Membership
                // is otherwise derived from documents, and an item that has just
                // been created has none -- so without this the company that
                // typed it could not see it in the very picker it was created
                // from. Mode stays Default, which tracking ignores, so this
                // records ownership and changes no behaviour.
                await EnsureRegisteredAsync(companyId.Value, created.Id);
            if (companyId.HasValue)
        ...
        public async Task<ItemTypeDto?> UpdateAsync(int id, ItemTypeDto dto, int? companyId = null)
        {
            ...
            it.HsCodeId = await ResolveHsCodeIdAsync(normalizedHs);
            it.UOM = dto.UOM;
            ...
            if (companyId.HasValue && dto.WriteCompanyOverlay)
                ...
                await EnsureRegisteredAsync(companyId.Value, updated.Id);
            if (companyId.HasValue)
        ...
        /// </summary>
        /// <summary>
        /// Record that this company has this catalog row on its books, without
        /// touching the division / GL overlay fields. Idempotent.
        /// </summary>
        private async Task EnsureRegisteredAsync(int companyId, int itemTypeId)
        {
            var exists = await _context.CompanyItemTypeSettings
                .AnyAsync(s => s.CompanyId == companyId && s.ItemTypeId == itemTypeId);
            if (exists) return;
            _context.CompanyItemTypeSettings.Add(new CompanyItemTypeSetting
            {
                CompanyId = companyId,
        ...
        /// </summary>
        /// <summary>
        /// Id of the HS master row for this code, or null when the code is blank
        /// or the master does not (yet) hold it. Never throws — validation is
        /// <see cref="ValidateHsCodeOrThrowAsync"/>'s job.
        /// </summary>
        private async Task<int?> ResolveHsCodeIdAsync(string? hsCode)
        {
            if (string.IsNullOrWhiteSpace(hsCode)) return null;
            var code = hsCode!.Trim();
            return await _context.HsCodes.AsNoTracking()
                .Where(h => h.Code == code)
                .Select(h => (int?)h.Id)
                .FirstOrDefaultAsync();
        }
        ...
        private async Task ValidateHsCodeOrThrowAsync(string? hsCode, int? companyIdHint)
        {
            ...
            var candidate = hsCode!.Trim();

            // Local HS master first (2026-08-30). Once the tariff has been
            // imported, this is the authority — and it answers with no FBR token
            // and no regard for whether the company has FBR integration on,
            // which is exactly what classifying item types must not depend on.
            // An empty master (never imported) falls through to the old
            // PRAL-backed path below, so FBR-enabled tenants lose nothing.
            var masterHasRows = await _context.HsCodes.AnyAsync(h => h.IsActive);
            if (masterHasRows)
            {
                var known = await _context.HsCodes
                    .AnyAsync(h => h.IsActive && h.Code == candidate);
                if (known) return;

                throw new InvalidOperationException(
                    $"HS Code '{candidate}' is not in the HS code master. " +
                    "Pick a code from the autocomplete, or run Import HS Codes on the Item Types " +
        ...
        private async Task EnrichFromFbrAsync(ItemTypeDto dto, int? companyId)
        {
            ...
            // Local HS master first (2026-08-30): when the imported tariff row
            // already knows this code's UOM, use it. Works with no company
            // context and no FBR token at all, which is the point — an operator
            // whose company has FBR integration off still gets the UOM filled in.
            var code = dto.HSCode!.Trim();
            var master = await _context.HsCodes.AsNoTracking()
                .FirstOrDefaultAsync(h => h.Code == code);
            if (master != null && !string.IsNullOrWhiteSpace(master.Uom))
            {
                if (!dto.FbrUOMId.HasValue) dto.FbrUOMId = master.FbrUomId;
                if (string.IsNullOrWhiteSpace(dto.UOM)) dto.UOM = master.Uom;
                return;
            }

            // Master has no UOM for this code yet — fall back to asking FBR
            // through the company's own token, as before.
