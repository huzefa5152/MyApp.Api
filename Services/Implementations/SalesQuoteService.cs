...
namespace MyApp.Api.Services.Implementations
{
    ...
    /// <see cref="ConvertToSalesOrderAsync"/>. QuoteNumber is unique per company.
    /// </summary>
    public class SalesQuoteService : ISalesQuoteService
    {
        ...
        public async Task<SalesQuoteDto> CreateAsync(int companyId, SalesQuoteDto dto)
        {
            ...
            // Remember the line descriptions in the generic item catalog so
            // they become reusable suggestions on future documents. Best-effort:
            // the quote is already committed — a catalog hiccup must not fail it.
        ...
        public async Task<bool> DeleteAsync(int id)
        {
            ...
            // order itself SURVIVES — it just loses its quote reference. Quotes
            // are pre-sale, non-FBR documents, so deleting one (even a converted,
            // non-latest quote) is allowed and a numbering gap is acceptable here.
        ...
        public async Task<SalesOrderDto> ConvertToSalesOrderAsync(int id)
        {
            ...
            // A quote auto-expires once its validity date has passed; an expired
            // quote can no longer be converted (it's dropped from the order-form
            // picker too). The operator creates a fresh quote instead.
            if (quote.ValidUntil.HasValue && quote.ValidUntil.Value.Date < DateTime.UtcNow.Date)
                throw new InvalidOperationException("This quote has expired and can no longer be converted to a sales order.");
        ...
        private async Task RememberDescriptionsAsync(IEnumerable<string?> descriptions)
        {
            ...
            {
                _logger.LogWarning(ex, "Failed to upsert sales-quote item descriptions into the catalog (non-fatal).");
            }
        }
