...
namespace MyApp.Api.Services.Interfaces
{
    public interface IAttachmentService
    {
        ...
        /// <param name="allowedDivisionIds">Division-RBAC scope (IDivisionAccessGuard):
        /// non-null hides rows tagged with divisions outside the set; company-level
        /// rows (null DivisionId) stay visible — policy D1. Null = unrestricted.</param>
        Task<List<AttachmentDto>> GetByFolderAsync(int companyId, int folderId, HashSet<int>? allowedDivisionIds = null);
        /// <summary>Attachments not filed in any folder (the "Uncategorized" bucket), disk-reconciled.</summary>
        Task<List<AttachmentDto>> GetUncategorizedAsync(int companyId, HashSet<int>? allowedDivisionIds = null);
        Task<List<AttachmentDto>> GetByEntityAsync(int companyId, string entityType, int entityId, HashSet<int>? allowedDivisionIds = null);
        /// <summary>entityId → attachment count (disk-reconciled), for list-card badges (e.g. Sales Quote list).</summary>
        Task<Dictionary<int, int>> GetCountsByEntityAsync(int companyId, string entityType, IEnumerable<int> entityIds, HashSet<int>? allowedDivisionIds = null);
        /// <summary>folderId → attachment count (disk-reconciled), for the Folders list badges.</summary>
        Task<Dictionary<int, int>> GetCountsByFolderAsync(int companyId, IEnumerable<int> folderIds, HashSet<int>? allowedDivisionIds = null);
