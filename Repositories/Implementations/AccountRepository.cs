    ...
    public class AccountRepository : IAccountRepository
    {
        ...
        public async Task<List<Account>> GetAccountsAsync(int companyId) =>
            await _context.Accounts
                ...
                // The group comes along so every account picker can show which
                // group an account sits in (AccountDto.AccountGroupName).
        ...
        public async Task<int> NextGroupPositionAsync(int companyId, FinancialStatement statement, int? parentGroupId)
        {
            ...
            q = parentGroupId.HasValue
                ? q.Where(g => g.ParentGroupId == parentGroupId.Value)
                : q.Where(g => g.ParentGroupId == null);
