...
try:
    print("===== 1. setup")
    # An item type to bill against. A freshly migrated database has none (the
    # HS tariff import has not run), so the suite makes its own rather than
    # assuming the catalogue is populated — that is what lets it run on any
    # branch and against a scratch database.
    found = call("GET", "/itemtypes?includeAutoGenerated=true&search=valve", None, TOKEN)[1] or []
    if found:
        item_type = found[0]
    else:
        st, item_type = call("POST", "/itemtypes", {
            "name": "_bulk suite valve", "hsCode": "8481.8090", "uom": "Numbers, pieces, units",
        }, TOKEN)
        assert st in (200, 201), f"could not create an item type: {st} {item_type}"
    print(f"  item type: {item_type['name']} (id {item_type['id']})")
    ...
    # The money a template needs, whichever production line this is. The
    # whole-rupee "…Rounded" set exists on some branches and not others, so it
    # is checked for CONSISTENCY rather than presence — a DTO that offers a
    # rounded total must also offer the words that go with it, or a rounded
    # template contradicts its own total row.
    pd = multi_entry["printData"]
    core = [k for k in ("subtotal", "gstAmount", "grandTotal", "items") if k in pd]
    check("the print data carries the money a template needs", len(core) == 4,
          f"missing {set(('subtotal','gstAmount','grandTotal','items')) - set(core)}")
    check("a rounded total comes with rounded words",
          ("grandTotalRounded" in pd) == ("amountInWordsRounded" in pd),
          f"rounded total={'grandTotalRounded' in pd} words={'amountInWordsRounded' in pd}")
