...
for (const t of templates) {
  ...
  check(`${t.label}: renders Net Payable`, /Net Payable/.test(text));
  ...
  check(`${t.label}: silent when there is no withholding`, !/Withholding Income Tax|Net Payable/.test(plain));
...
// The block nests one {{#if withholdingTaxRate}}, so strip up to the SECOND {{/if}}.
const BLOCK_RE = /\{\{#if withholdingTaxAmount\}\}[\s\S]*?\{\{\/if\}\}[\s\S]*?\{\{\/if\}\}\s*/g;
for (const t of templates) {
  const bare = t.html.replace(BLOCK_RE, "");
  ...
  if (r.changed) {
    ...
    check(`${t.label}: re-injected block renders`, /Net Payable/.test(text));
  }
}
