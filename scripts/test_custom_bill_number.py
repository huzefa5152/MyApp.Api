...
What this pins:
  ...
  • Numbering is PER DIVISION on this line, so the endpoint and both create
    paths are exercised against the company-level sequence AND a division's
    own — a number used in one division must stay free in the other.
...
def make_standalone(base, token, company, client, item_type, number=None, qty=1,
                    price=100, division_id=None):
    line = {
        "description": "Widget",
        "quantity": qty,
...
def next_number(base, token, company_id, check_value=None, division_id=None):
    parts = []
    if division_id is not None:
        parts.append(f"divisionId={division_id}")
    if check_value is not None:
        parts.append(f"check={check_value}")
...
def suite_next_number(base, token, company, prefix):
    ...
    check(s, "the demo band (900000) is refused",
          ...
          and "reserved" in (d.get("checkedError") or "").lower(),
          f"{status} {d}")
...
# ── Suite 5: per-division numbering ─────────────────────────────────────────
def suite_divisions(base, token, company, client, item_type):
    s = "5. Per-division numbering"
    print(f"\n=== {s} ===")
    cid = company["id"]
    created = []

    # Snapshot the company-level sequence: nothing done inside a division may
    # move it. (Its value depends on what the earlier suites issued, so it is
    # read rather than assumed.)
    status, before = next_number(base, token, cid)
    company_level_before = before.get("nextNumber") if status == 200 else None

    status, div = http("POST", f"/api/divisions/company/{cid}", base, token=token, body={
        "companyId": cid,
        "name": "North",
        "code": "N",
        "startingInvoiceNumber": 300,
    })
    if status not in (200, 201) or not isinstance(div, dict) or not div.get("id"):
        check(s, "division created", False, f"{status} {err_text(div)}")
        return created
    check(s, "division created", True)

    # The division starts on its OWN seed, not the company's.
    status, d = next_number(base, token, cid, division_id=div["id"])
    check(s, "a division's first bill offers the division's starting number",
          status == 200 and d.get("nextNumber") == 300, f"{status} {d}")

    # 7777 is taken at company level; it must still be free in the division,
    # because the unique key is (CompanyId, DivisionId, NoteKind, InvoiceNumber).
    status, d = next_number(base, token, cid, check_value=7777, division_id=div["id"])
    check(s, "a company-level number is still free inside a division",
          status == 200 and d.get("checkedAvailable") is True, f"{status} {d}")

    status, inv = make_standalone(base, token, company, client, item_type,
                                  number=7777, division_id=div["id"])
    if check(s, "the division can issue that number",
             status in (200, 201) and inv.get("invoiceNumber") == 7777,
             f"{status} {err_text(inv)}"):
        created.append(inv["id"])

    # ...and now it is taken THERE, but the company-level sequence is untouched.
    status, inv = make_standalone(base, token, company, client, item_type,
                                  number=7777, division_id=div["id"])
    check(s, "a duplicate within the division is refused",
          status == 400 and "7777" in err_text(inv), f"{status} {err_text(inv)}")
    if status in (200, 201):
        created.append(inv["id"])

    status, d = next_number(base, token, cid)
    check(s, "the company-level sequence did not move",
          status == 200 and d.get("nextNumber") == company_level_before,
          f"was {company_level_before}, now {d.get('nextNumber')}")

    return created


# ── Suite 6: renumbering an existing bill ───────────────────────────────────
def suite_edit(base, token, company, client, item_type, prefix):
    s = "6. Renumbering on edit"
    print(f"\n=== {s} ===")
    cid = company["id"]
    created = []
...
def suite_edit_fbr_lock(base, token, company, client, item_type, db):
    ...
    s = "7. A filed bill cannot be renumbered"
    print(f"\n=== {s} ===")
    created = []
    if not db:
    ...
    # This line has no "Submitting" / "Uncertain" states — the atomic submit
    # claim was never ported here — so an IRN is what the gate turns on.
    for label, st, irn in [
        ("Validated but already carrying an IRN", "Validated", "IRN-TEST-0001"),
        ("Failed but already carrying an IRN", "Failed", "IRN-TEST-0002"),
    ]:
        set_status(st, irn)
...
def main() -> int:
    ...
    try:
        ...
        suite_divisions(args.base, token, company, client, item_type)
