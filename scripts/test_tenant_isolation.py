...
# Overridable so the suite can be pointed at a second instance without
# editing it (MYAPP_ADMIN_USER / _PW below already work this way).
BASE = os.environ.get("MYAPP_BASE", "http://localhost:5134").rstrip("/")
...
# ── Suite 18: the item-type catalog is scoped to the caller's companies ──
#
# ItemType is a GLOBAL table with no CompanyId -- right for the HS tariff
# master, wrong for the rows an operator typed. On an installation serving
# unrelated tenants every picker was listing every other tenant's product
# names, because passing companyId only ever changed the SORT.
#
# Membership is derived (registrations + documents + stock), so these checks
# are what stop a future query forgetting a table and either leaking a name or
# stranding an item its owner can no longer see.
suite = "item-type catalog scoping"
print(f"\n=== {suite} ===")

def _itemtypes(token, company_id=None, search=None, paged=False, include_auto=False):
    qs = []
    if company_id is not None:
        qs.append(f"companyId={company_id}")
    if search:
        qs.append("search=" + urllib.parse.quote(search))
    if include_auto:
        qs.append("includeAutoGenerated=true")
    if paged:
        qs.append("pageSize=200")
    path = "/api/itemtypes" + ("/paged" if paged else "") + ("?" + "&".join(qs) if qs else "")
    st, body = request("GET", path, token=token)
    rows = (body or {}).get("items", []) if paged else (body or [])
    return st, (rows if isinstance(rows, list) else [])

# Alpha types an item. Nothing else references it yet, so it exists ONLY by
# virtue of the registration the create path writes -- which is the case that
# breaks if creation stops recording ownership.
alpha_item_name = f"ISO SCOPE ALPHA {uuid.uuid4().hex[:6]}"
st, alpha_item = request("POST", f"/api/itemtypes?companyId={alpha['id']}", token=tokens["alice"],
                         body={"name": alpha_item_name, "uom": "Numbers, pieces, units",
                               "isFavorite": True})
check(suite, "Alpha can create an item type", st in (200, 201), f"http {st}: {alpha_item}")

if st in (200, 201):
    st_a, rows_a = _itemtypes(tokens["alice"], alpha["id"], search="ISO SCOPE ALPHA")
    check(suite, "Alpha sees the item it just typed -- before any document uses it",
          st_a == 200 and any(r["id"] == alpha_item["id"] for r in rows_a),
          "a freshly created item was invisible to its own creator")

    st_b, rows_b = _itemtypes(tokens["bob"], beta["id"], search="ISO SCOPE ALPHA")
    check(suite, "Beta cannot see Alpha's item in the picker",
          st_b == 200 and not any(r["id"] == alpha_item["id"] for r in rows_b),
          f"LEAK: Beta was offered {[r['name'] for r in rows_b]}")

    st_p, rows_p = _itemtypes(tokens["bob"], beta["id"], search="ISO SCOPE ALPHA", paged=True)
    check(suite, "nor in the paged Item Catalog",
          st_p == 200 and not any(r["id"] == alpha_item["id"] for r in rows_p),
          f"LEAK via /paged: {[r['name'] for r in rows_p]}")

    # Bob can reach only Beta, so his no-companyId call must not widen to Alpha.
    st_n, rows_n = _itemtypes(tokens["bob"], None, search="ISO SCOPE ALPHA")
    check(suite, "nor by omitting companyId altogether",
          st_n == 200 and not any(r["id"] == alpha_item["id"] for r in rows_n),
          f"LEAK via the unscoped list: {[r['name'] for r in rows_n]}")

    # Carol reaches both companies, so she is entitled to it -- the filter is
    # tenant scope, not per-company blinkers, and a suite that only proved
    # things were hidden would pass just as well if everything were hidden.
    st_c, rows_c = _itemtypes(tokens["carol"], alpha["id"], search="ISO SCOPE ALPHA")
    check(suite, "a user who can reach Alpha still sees it",
          st_c == 200 and any(r["id"] == alpha_item["id"] for r in rows_c),
          "scoping hid the item from a user entitled to it")

# An un-adopted HS-import placeholder is FBR's published tariff, not tenant
# data. It has to stay visible to everyone or a company could not classify its
# first item.
st, cat = request("GET", "/api/itemtypes/paged?pageSize=200&search=.", token=admin)
placeholder = next((r for r in ((cat or {}).get("items") or [])
                    if r.get("isAutoGenerated") and not r.get("isFavorite")
                    and r.get("hsCode")), None)
if placeholder is None:
    check(suite, "tariff placeholder available to test with", True,
          "skipped -- no HS tariff loaded on this installation")
else:
    seen = []
    for who, comp in (("alice", alpha), ("bob", beta)):
        st_t, rows_t = _itemtypes(tokens[who], comp["id"], search=placeholder["hsCode"],
                                  include_auto=True)
