...
def suite_customer_refund(base, token, cid, bank_id, ar_id, client):
    ...
    # A refund must also be VISIBLE on the customer's trail. It debits their A/R
    # in the GL, so a ledger that omitted it would disagree with the books and
    # with the A/R column. This shape is new: before the payee change a money-out
    # payment naming a Client could only reach an income/expense account.
    st, stmt = http("GET", f"/api/clients/{client['id']}/statement", base, token=token)
    rows = (stmt or {}).get("entries") or [] if st == 200 else []
    ref = [r for r in rows if r.get("type") == "Refund" and eq(r.get("credit"), 5000)]
    check(s, "the refund appears on the customer's ledger as a credit", bool(ref),
          f"{len(rows)} rows, none a 5000 Refund credit")
...
def suite_advance_visible_to_reports(base, token, cid, bank_id, client):
    ...
    The ledger recorded advances correctly from the start, but the A/R column on
    the Clients screen and the aged-receivables report both derived a customer's
    position from invoices alone, so an advance with no invoice was invisible.

    COLUMN CONVENTION — this branch's customer statement is served by
    ICustomerLedgerService, which follows the operator's workbook: an invoice is a
    CREDIT and the money that settles it a DEBIT, and a receipt is reported at its
    FULL Payment.Amount. An advance is therefore already a plain "Receipt" row on
    the money-in side rather than a separate "Advance received" type. The
    invariant pinned below is the one that matters either way: the advance is on
    the trail at its full amount, it moves the closing balance by exactly that
    amount, and the A/R column agrees with the trail to the paisa.
    """
    ...
    adv = [r for r in rows if eq(r.get("debit"), 1000) and "RCP-" in (r.get("reference") or "")]
    check(s, "the advance appears on the statement", bool(adv),
          f"{len(rows)} rows, none debiting 1000")
    if adv:
        # Money in, at the FULL amount, with nothing on the other side — the row
        # that used to be missing entirely when no invoice backed the cash.
        check(s, "it is money-in at its full amount and nothing else",
              adv[0].get("type") == "Receipt" and eq(adv[0].get("credit"), 0),
              f"type={adv[0].get('type')} credit={adv[0].get('credit')}")
