...
def db_set_fbr_status(server: str, db: str, invoice_id: int, status: str) -> bool:
    # SELECT @@ROWCOUNT so a poke against the wrong database is a hard failure
    # rather than a silent no-op that fails the assertion further down.
    q = (f"SET NOCOUNT ON; UPDATE Invoices SET FbrStatus = '{status}' "
         f"WHERE Id = {int(invoice_id)}; SELECT @@ROWCOUNT;")
    try:
        ...
        for tok in (r.stdout or "").split():
            if tok.strip().isdigit():
                affected = int(tok.strip())
                break
        if affected < 1:
            print(f"      (poke matched NO rows: invoice {invoice_id} is not in "
                  f"[{db}] on [{server}] -- pass --db-name/--db-server for the "
                  f"database this API is actually running against)")
            return False
        return True
...
def setup(base: str, admin_user: str, admin_pw: str, fbr_token: str,
    ...
    # This branch will not accept an unclassified bill line, so the item the
    # bill sells has to exist in the catalog first. Same HS code the payload
    # declares, so HS validation resolves against the master rather than the
    # line's own string.
    status, item_type = http("POST", f"/api/itemtypes?companyId={company_id}", base, token=token, body={
        "name": f"Pneumatic Solenoid Valve 220VAC 6VA {suffix}",
    ...
    if status not in (200, 201):
        sys.exit(f"FATAL: create item type failed ({status} {item_type})")
