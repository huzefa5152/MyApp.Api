...
# This branch's rule is ALL-OR-NOTHING, not trimming: a group that spans any
# company the caller cannot reach is withheld entirely, so its display name and
# NTN cannot leak (ClientGroupService.AccessibleGroupIdsAsync). NTN 7654321 now
# spans A1, A2 AND B1, so A still sees nothing -- which is the stricter answer,
# and deliberately different from the trader line that trims the group to the
# caller's own members.
s, rows = request("GET", f"/api/clients/common?companyId={coA1['id']}", token=tA)
check("common", "a group spanning B1 stays hidden from A even once A shares it twice",
      s == 200 and rows == [], f"{s} {rows}")

# A group wholly inside A's own companies IS visible, which is what proves the
# rule above is scoping rather than simply breaking the feature.
mk_client(tA, coA1["id"], "Scope Own Client", "7654322")
mk_client(tA, coA2["id"], "Scope Own Client", "7654322")
s, rows = request("GET", f"/api/clients/common?companyId={coA1['id']}", token=tA)
own = [r for r in rows if set(r["companyNames"]) == {coA1["name"], coA2["name"]}] if s == 200 else []
check("common", "A sees a group that is wholly its own (A1 + A2)",
      s == 200 and len(own) == 1 and own[0]["companyCount"] == 2, f"{s} {rows}")
gid = own[0]["groupId"] if own else None
if gid:
    ...
    # The by-id routes were UNSCOPED on this branch until 2026-09-15: knowing a
    # group id was enough to read another tenant's master record, rename it
    # across every company, or delete it across every company. Now 404.
    s, d = request("GET", f"/api/clients/common/{gid}", token=tB)
    check("common", "B cannot read A's group by id", s == 404, f"{s} {d}")
    s, d = request("PUT", f"/api/clients/common/{gid}", token=tB, body={"name": "Hijacked", "ntn": "7654322", "address": "y", "phone": "1", "email": "", "strn": "", "registrationType": "Registered", "cnic": "", "fbrProvinceCode": 8})
    check("common", "B cannot rename A's group by id", s == 404, f"{s} {d}")
    s, d = request("DELETE", f"/api/clients/common/{gid}", token=tB)
    check("common", "B cannot delete A's group by id", s == 404, f"{s} {d}")
    s, d = request("PUT", f"/api/clients/common/{gid}", token=tA, body={"name": "Scope Own Client Renamed", "ntn": "7654322", "address": "y", "phone": "1", "email": "", "strn": "", "registrationType": "Registered", "cnic": "", "fbrProvinceCode": 8})
    check("common", "A edits its own common group -> 200", s == 200, f"{s} {d}")
