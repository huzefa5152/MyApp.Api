...
# Every HS code this suite's workbooks carry. ItemType is a GLOBAL catalog with
# no CompanyId (CLAUDE.md 5b-2b), so deleting the throwaway companies does NOT
# undo what an import commit did to the catalog: it ADOPTS the tariff
# placeholder under a code -- renames it to the sheet's product name and ticks
# IsFavorite -- and that survives the company that caused it.
#
# The suite then eroded itself. "every item is new on a first upload" asserts
# that a first import creates its items; on the second run the placeholders were
# already renamed, so the match ladder answered `matched-renamed` for three of
# the four and the check failed against a catalog its own previous run had
# rewritten. Nothing was wrong with the importer.
#
# So: snapshot these codes before anything runs, put them back afterwards.
FIXTURE_HS_CODES = ["8481.1000", "8513.1090", "8450.9000", "8712.0000"]


def item_types_under(api, h, code):
    """Every item type carrying this HS code that the caller can SEE.

    /itemtypes/paged rather than /itemtypes?search=, because the latter is
    ranked and capped. But the important limit applies to both: item-type
    visibility is DERIVED from the caller's accessible companies (CLAUDE.md
    5b-2b), so a row whose only company has been deleted is visible to nobody
    -- it is not an un-adopted tariff placeholder, and no company references it
    any more.

    That is why the teardown reads this list BEFORE it deletes the companies
    and then deletes by ID afterwards. Reading it after, as the first cut did,
    finds nothing and leaves one orphan behind per run -- silently, since the
    same invisibility hides them from every screen too.
    """
    found, page = [], 1
    while page <= 20:                       # 2,000 rows under one code is not a thing
        try:
            r = requests.get(f"{api}/itemtypes/paged", headers=h, timeout=60,
                             params={"search": code, "page": page, "pageSize": 100})
            if r.status_code != 200:
                break
            body = r.json() or {}
            items = body.get("items") or []
            found.extend(it for it in items if (it.get("hsCode") or "") == code)
...
def snapshot_item_types(api, h):
    """id -> the fields an import can change, for every item type under a
    fixture HS code. Keyed by id so a row RENAMED by this run is still found."""
    snap = {}
    for code in FIXTURE_HS_CODES:
        for it in item_types_under(api, h, code):
            snap[it["id"]] = {
                "name": it.get("name") or "",
                "hsCode": it.get("hsCode"),
                "uom": it.get("uom"),
                "saleType": it.get("saleType"),
                "isFavorite": bool(it.get("isFavorite")),
            }
    return snap


def plan_item_type_restore(api, h, snap):
    """What the teardown will have to undo -- computed while the run's
    companies still EXIST, because that is the only time its item types can be
    found (see item_types_under).

    Returns (ids_to_delete, rows_to_rename).
    """
    to_delete, to_rename = [], []
    for code in FIXTURE_HS_CODES:
        for it in item_types_under(api, h, code):
            ...
            elif (it.get("name") or "") != was["name"] or bool(it.get("isFavorite")) != was["isFavorite"]:
                to_rename.append(was | {"id": it["id"]})
    return to_delete, to_rename


def restore_item_types(api, h, plan):
    """Delete what this run created; put back what it renamed or adopted.

    Runs AFTER the companies are gone -- they hold the documents that reference
    an item type, so the delete would be refused while they exist. Addresses
    every row by id for the reason item_types_under records.

    Best-effort throughout: a teardown that fails the run would hide the result
    the run exists to report. A delete the server refuses (something else has
    since adopted the row) is left alone on purpose.
    """
    to_delete, to_rename = plan
    for iid in to_delete:
        try:
            requests.delete(f"{api}/itemtypes/{iid}", headers=h, timeout=60)
        except Exception:
            pass
    for was in to_rename:
        # A tariff placeholder this run adopted: rename it back and un-tick
        # IsFavorite, which is the adoption signal every picker reads
        # (CLAUDE.md 5b-2). IsAutoGenerated is never cleared, so the row goes
        # back to being the un-adopted placeholder it was.
        try:
            requests.put(f"{api}/itemtypes/{was['id']}", headers=h, timeout=60, json={
                "id": was["id"],
                "name": was["name"],
                "hsCode": was["hsCode"],
                "uom": was["uom"],
                "saleType": was["saleType"],
                "isFavorite": was["isFavorite"],
            })
        except Exception:
            pass
...
def main():
    ...
    # BEFORE anything imports. The global catalog is shared state this suite
    # rewrites; the snapshot is what lets the finally block hand it back.
    item_type_snapshot = snapshot_item_types(api, h)
    ...
    try:
        ...
        # A first upload must not land on any item an OPERATOR already owns.
        # It may still land on the tariff PLACEHOLDER for the code, and on a
        # tariff-loaded installation it always will -- that is what a
        # placeholder is for (CLAUDE.md 5b-2: adoption is the signal). So the
        # property to pin is "four distinct items, none of them somebody's
        # existing product", not "four rows said will-create": the strict form
        # asserted the state of the HS master rather than the importer, and
        # went red on any database where the tariff had been imported.
        sc = prev.get("statusCounts", {})
        fresh = sc.get("will-create", 0) + sc.get("matched-renamed", 0)
        reused = sc.get("matched", 0) + sc.get("matched-classified", 0) + sc.get("ambiguous", 0)
        check("every item is new on a first upload",
              fresh == 4 and reused == 0, f"counts={sc}")
        ...
        # Created OR adopted, for the reason above: a placeholder the import
        # renames counts as updated, not created, and which of the two happens
        # depends on whether this installation carries the tariff.
        check("four item types are created",
              (res.get("itemTypesCreated", 0) + res.get("itemTypesUpdated", 0)) == 4,
        ...
        if not args.keep:
            # Work out what to undo while the companies still exist -- an item
            # type created by a company that has been deleted is invisible to
            # every API (see item_types_under), so this cannot wait.
            restore_plan = plan_item_type_restore(api, h, item_type_snapshot)
            ...
            # type, and the catalog rows cannot go until those do. Only then put
            # the global catalog back the way this run found it.
            restore_item_types(api, h, restore_plan)
