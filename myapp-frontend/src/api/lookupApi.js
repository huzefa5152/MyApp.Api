...
export const searchItemDescriptions = (query, companyId) =>
  httpClient.get("/lookup/items", { params: { query, companyId } });

// Exact-name lookup — returns the ItemDescription row with FBR defaults
// (HSCode, SaleType, FbrUOMId, UOM) or 404 if not found.
export const getItemByName = (name, companyId) =>
  httpClient.get("/lookup/items/by-name", { params: { name, companyId } });

export const searchUnits = (query, companyId) =>
  httpClient.get("/lookup/units", { params: { query, companyId } });
...
export const saveItemFbrDefaults = (payload) =>
  httpClient.post("/lookup/items/fbr-defaults", payload, { params: { companyId: payload.companyId } });
