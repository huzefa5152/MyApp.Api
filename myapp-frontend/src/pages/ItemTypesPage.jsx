...
import { MdCategory, MdAdd, MdEdit, MdDelete, MdSearch, MdStar, MdStarBorder, MdInfo, MdBusiness, MdCloudDownload } from "react-icons/md";
...
export default function ItemTypesPage() {
  ...
  // operator can find one and rename it.
  ...
  const fetchAll = async (cid = companyId, opts = {}) => {
    const withPlaceholders = opts.showPlaceholders ?? showPlaceholders;
    ...
    try {
      ...
      const { data } = await getItemTypes(cid || undefined,
        withPlaceholders ? { includeAutoGenerated: true, search: term, take: 200 } : {});
      setItemTypes(data);
    } catch {
  ...
  // and when the placeholder toggle flips (that changes what the server sends).
  useEffect(() => { fetchAll(companyId); }, [companyId, showPlaceholders]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div>
      ...
      {(itemTypes.length > 5 || showPlaceholders) && (
        <div style={styles.searchWrap}>
          <MdSearch size={18} style={styles.searchIcon} />
          <input
            type="text"
            placeholder={showPlaceholders
              ? "Search placeholders by name or HS code, then press Enter"
              : "Search by name, HS code, UOM…"}
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            /* With placeholders shown the list is capped server-side, so the
               search has to go to the server rather than filter what's loaded. */
            onKeyDown={(e) => { if (e.key === "Enter" && showPlaceholders) fetchAll(companyId); }}
            style={styles.searchInput}
          />
        </div>
      )}

      <label style={styles.placeholderToggle}>
        <input
          type="checkbox"
          checked={showPlaceholders}
          onChange={(e) => { setSearch(""); setShowPlaceholders(e.target.checked); }}
        />
        <span>
          Show HS-code placeholders
          <span style={styles.placeholderHint}>
            Items the HS import created, named “HS Code 6109.1000”. Hidden by default so they
            stay out of this list and the document pickers — switch this on to find one and rename it.
            {showPlaceholders ? " Showing the first 200 matches." : ""}
          </span>
        </span>
      </label>
...
const styles = {
  ...
  placeholderToggle: { display: "flex", gap: "0.5rem", alignItems: "flex-start", marginBottom: "1rem", fontSize: "0.84rem", color: colors.textPrimary, cursor: "pointer", maxWidth: 720 },
  placeholderHint: { display: "block", fontSize: "0.78rem", color: colors.textSecondary, marginTop: "0.15rem", lineHeight: 1.4 },
