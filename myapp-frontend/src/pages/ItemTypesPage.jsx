...
import { MdCategory, MdAdd, MdEdit, MdDelete, MdSearch, MdStar, MdStarBorder, MdInfo, MdBusiness, MdCloudDownload, MdClose } from "react-icons/md";
import { getItemTypesPaged, updateItemType, deleteItemType } from "../api/itemTypeApi";
...
export default function ItemTypesPage() {
  ...
  // HS code master is reference data, so importing it is its own permission —
  // it is not part of itemtypes.manage.* and has nothing to do with FBR being
  // enabled on the selected company.
  const canImportHsCodes = has("hscodes.import.run");
  ...
  // tariff import creates and items brought in by a spreadsheet import all
  // appear here together — there is no "show placeholders" switch, because
  // splitting the catalog in two is what made an imported item look missing.
  //
  // That means thousands of rows, so paging and searching are both SERVER-side:
  // the page holds one page at a time and never filters what it already has.
  ...
  const fetchAll = async (opts = {}) => {
    ...
    const term = opts.search ?? debouncedSearch;
    const wanted = opts.page ?? page;
    setLoading(true);
    try {
      ...
      const { data } = await getItemTypesPaged(cid || undefined, {
        search: term, page: wanted, pageSize,
      });
      setItemTypes(data.items || []);
      setTotal(data.totalCount || 0);
      setTotalPages(Math.max(1, data.totalPages || 1));
      // A delete, or a narrower search, can strand the operator past the end.
      if ((data.items || []).length === 0 && wanted > 1)
        setPage(Math.min(wanted - 1, Math.max(1, data.totalPages || 1)));
    } catch {
  ...
  // Typing should not fire a request per keystroke now that the search runs on
  // the server, and a new term always starts again from page 1.
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  // Reload when the company changes (the overlay columns are company-specific),
  // when the page or rows-per-page changes, or on a new search term.
  useEffect(() => { fetchAll(); }, [companyId, page, pageSize, debouncedSearch]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // No client-side filter: the search term goes to the server, so what is
  // loaded IS the result. Filtering the page again would search one page of a
  // catalog that has thousands.
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
          ...
          <div>
            ...
            <p style={styles.subtitle}>
              ...
              {" · "}added by hand, HS import and spreadsheet import together
            </p>
          </div>
        </div>
      ...
      <div style={styles.searchWrap}>
        ...
        />
        {search && (
          <button type="button" style={styles.searchClear} onClick={() => setSearch("")}
                  title="Clear search">
            <MdClose size={16} />
          </button>
        )}
                ...
                <div key={it.id} style={styles.item}>
                  ...
                  <div style={{ flex: 2, minWidth: 0 }}>
                    <div style={styles.itemName}>
                      {it.name}
                      {it.isAutoGenerated && !it.isFavorite && (
                        <span style={styles.originBadge}
                              title="Created by the HS code import. Rename it, give it a unit, and star it to offer it in the document pickers.">
                          HS import
                        </span>
                      )}
                    </div>
              ...
              <div key={it.id} className="it-card">
                <div className="it-card__top">
                  ...
                  <div className="it-card__title">
                    <div className="it-card__name">
                      {it.name}
                      {it.isAutoGenerated && !it.isFavorite && (
                        <span style={styles.originBadge}>HS import</span>
                      )}
                    </div>
                    {it.fbrDescription && (
      ...
      {showHsImport && (
        <HsCodeImportModal
          /* Only used to fall back to this company's own FBR token when the
             installation has no reference token yet — the imported codes are
             shared by every company either way. */
          companyId={companyId}
...
const styles = {
  ...
  searchClear: { position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", display: "grid", placeItems: "center", width: 28, height: 28, border: "none", borderRadius: 6, background: "transparent", color: colors.textSecondary, cursor: "pointer" },
