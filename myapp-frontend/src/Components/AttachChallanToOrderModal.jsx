 ...
 *               unlinked challans (the No-PO-before-PO case).
 ...
 * plus "Don't map (extra)". The delivered quantity was already booked when the
 * challan was created — attaching only links the rows and adopts the order's PO.
 */
export default function AttachChallanToOrderModal({ companyId, order, challan, onClose, onAttached }) {
  ...
  const buildAutoMap = useCallback((lines, items) => {
    ...
    for (const line of lines) {
      ...
      for (const pool of pools) {
        ...
        const byDesc = d && pool.find((s) => (s.description || "").trim().toLowerCase() === d);
  ...
  const challanOptions = useMemo(() => attachable.map((c) => {
    const items = (c.lines || []).map((l) => l.description).filter(Boolean);
            ...
            {fromOrder ? (
              <>
                ...
                {challanOptions.length > 0 && (
                  <div style={s.hint}>Only No-PO challans are shown. Search by challan # or item description; expand a row (▸) to see its items.</div>
                )}
              </>
            ) : (
              <SearchableSelect
...
// a common { deliveryItemId, itemTypeId, itemTypeName, description, quantity, unit }.
