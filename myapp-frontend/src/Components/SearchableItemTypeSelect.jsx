...
export default function SearchableItemTypeSelect({
  ...
  // Matches pulled from the FULL catalog while typing.
  //
  // The parent hands us the CURATED list only — placeholders created by the HS
  // import are excluded from /api/itemtypes by default and capped at 200 even
  // when included, deliberately, so 7,800 of them cannot drown every document
  // picker. The cost was that an imported code could not be reached at all:
  // filtering a list the server never sent finds nothing. Searching the server
  // as the operator types keeps the short curated default AND makes every code
  // findable.
  ...
  // Ask the server for catalog matches once the operator has typed enough to
  // mean something. Two characters, debounced — a shorter term would match
  // thousands of rows and tell them nothing.
  useEffect(() => {
    const term = query.trim();
    if (!open || term.length < 2) { setCatalogItems([]); setCatalogSearching(false); return; }

    let cancelled = false;
    setCatalogSearching(true);
    const timer = setTimeout(async () => {
      try {
        const { data } = await getItemTypes(effectiveCompanyId || undefined, {
          includeAutoGenerated: true, search: term, take: 50,
        });
        if (!cancelled) setCatalogItems(Array.isArray(data) ? data : []);
      } catch {
  ...
  }, [query, open, effectiveCompanyId]);

  // Parent list + inline-created types + catalog matches (deduped by id).
  const mergedItems = useMemo(() => {
    const base = items || [];
    const seen = new Set(base.map((i) => i.id));
    const extra = [
      ...createdItems.filter((c) => !seen.has(c.id)),
      ...catalogItems.filter((c) => !seen.has(c.id) && !createdItems.some((x) => x.id === c.id)),
    ];
    return extra.length ? [...base, ...extra] : base;
  }, [items, createdItems, catalogItems]);
