 ...
 * @param {string|number} divisionId  numbering is per division on this line
 ...
 * @param {string}   changeCaution   edit only: shown once the number is actually changed
    ...
    <div>
      ...
      {lockedReason ? (
        ...
        <>
          ...
          {isEdit && changeCaution && !probe?.unchanged && (number || "").trim() && (
            <div style={{ fontSize: "0.72rem", color: colors.warn, marginTop: "0.2rem",
                          display: "flex", alignItems: "flex-start", gap: "0.25rem" }}>
              <MdErrorOutline size={13} style={{ flexShrink: 0, marginTop: 2 }} />
              <span>{changeCaution}</span>
            </div>
          )}
        </>
      )}
